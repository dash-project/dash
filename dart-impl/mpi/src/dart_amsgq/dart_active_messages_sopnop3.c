...
int grequest_poll_fn(void *data, MPI_Status *status)
{
  ...
  switch (state->state) {
    ...
    {
      // TODO: do we need this step after all?
    ...
    {
      MPI_Win_flush(state->flush_info->target, state->amsgq->queue_win);

      // kick off registration
      uint64_t writecnt_offset = fuse(1, state->flush_info->size);
      ...
      MPI_Fetch_and_op(
        ...
        &state->fused_val,
        MPI_UINT64_T,
        state->flush_info->target,
        OFFSET_WRITECNT(state->queuenum),
        MPI_SUM,
    ...
    case DART_GREQUEST_REGISTER:
    {
      ...
      // wait for the fetch-and-op to complete
      // TODO: use MPI_Raccumulate instead?
  ...
  int                           num_info)
{
  ...
  for (int i = 0; i < num_info; ++i) {
    ...
    // fetch queue number
    MPI_Get(&states[i].queuenum, 1, MPI_INT64_T, flush_info[i].target,
             OFFSET_QUEUENUM, 1, MPI_INT64_T, amsgq->queue_win);
...
dart_ret_t
dart_amsg_sopnop3_init(dart_amsgq_impl_t* impl)
{

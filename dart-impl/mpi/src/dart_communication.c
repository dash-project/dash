...
dart_ret_t dart_alltoall(
  ...
  dart_team_t        team)
{
  CHECK_IS_BASICTYPE(sendtype);
  CHECK_IS_BASICTYPE(recvtype);

  MPI_Datatype mpi_stype = dart__mpi__datatype_struct(sendtype)->basic.mpi_type;
  MPI_Datatype mpi_dtype = dart__mpi__datatype_struct(recvtype)->basic.mpi_type;
  /*
   * MPI uses offset type int, do not copy more than INT_MAX elements:
   */
  if (dart__unlikely(

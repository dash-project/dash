  ...
  size_t nbytes)
{
...
#ifndef PROGRESS_ENABLE
  {
...
#else
    {
	    ...
	    if (seg_id){
		    dart_adapt_transtable_get_selfbaseptr(seg_id, &addr);
	    }else addr = dart_mempool_localalloc;
//	    MPI_Comm_rank (dart_sharedmem_comm_list[index], &sharedmem_rank);
...
*/
    ...
    DART_LOG_DEBUG("PUT  - %d bytes (allocated with local allocation) "
          "to %d at the offset %d\n", 
          nbytes, target_unitid_rel, offset);
    (*handle) -> dest = target_unitid_rel;
  }
  (*handle) -> request = mpi_req;
  (*handle) -> win     = win;
...
//        dart_sharedmem_size[j] - 1);
    ...
    if (is_sharedmem) {
      ...
      disp_rel = offset;
      baseptr += disp_rel;
  //    printf ("get intra blocking: baseptr[%d] = %d\n", 0, baseptr[0]);

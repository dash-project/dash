...
//	int test_ranks[4], all_ranks[4];
//	for (i = 0; i < 4; i++)
//		test_ranks[i] = i;
//	MPI_Group_translate_ranks(sub_group, 4, test_ranks, group_all, all_ranks);
...
//	MPI_Win_free (&(sharedmem_win_list[index]));
	...
	if (unitid == PROGRESS_NUM){
		int iter;
		for (iter = 0; iter < PROGRESS_NUM; iter++){
			MPI_Send (&dart_progress_index[index], 1, MPI_INT32_T, PROGRESS_UNIT+iter, TEAMDESTROY, dart_sharedmem_comm_list[0]);
		}
	}
#endif
#endif
	...
	DART_LOG_DEBUG ("%2d: TEAMDESTROY	- destroy team %d\n", id, teamid);
#ifdef SHAREDMEM_ENABLE
#ifdef PROGRESS_ENABLE
        }
#endif
...
//	printf ("globalid is %d\n", *globalid);
	...
	if (localid >= size) {
		DART_LOG_ERROR ("Invalid localid input: %d\n", localid);
		return DART_ERR_INVAL;
	}

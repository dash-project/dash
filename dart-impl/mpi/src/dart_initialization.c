   ...
   * each of which can create a shared memory region */
	...
	//	dart_teams[index] = user_comm_world;
		...
		if (user_comm_world != MPI_COMM_NULL){
			...
			MPI_Win_allocate_shared (DART_MAX_LENGTH, sizeof (char), MPI_INFO_NULL, sharedmem_comm,
...
//#endif
     ...
     * of DART_TEAM_ALL. */
      ...
      sizeof (int) * dart_sharedmemnode_size[index]);
		...
		for (i = 0; i < dart_sharedmemnode_size[index]; i++){
			dart_sharedmem_progress_table[index][dart_unit_mapping_progress[i]] = i;
		}
...
#endif
#endif
	DART_LOG_DEBUG("%2d: dart_init: Initialization finished\n", rank);
        _dart_initialized = 1;
...
{
  if (!_dart_initialized) {
    DART_LOG_DEBUG("dart_exit(): DART has not been initialized\n");
    return DART_ERR_OTHER;
  }
 // _dart_initialized = 0;
	...
	if (user_comm_world == MPI_COMM_NULL)
	{
		...
		while (1)
		{
			...
			//	printf ("progress processes are still here\n");
		...
		//			MPI_Bcast (&progress_index, 1, MPI_INT32_T, 0, newcomm);}
				...
				if (newcomm != MPI_COMM_NULL){
					...
					if (sharedmem_comm != MPI_COMM_NULL)
						MPI_Bcast (&progress_index, 1, MPI_INT32_T, 0, sharedmem_comm);
...
/*
			...
			//				dart_sharedmem_comm_list[0]);
				...
				MPI_Send (NULL, 0, MPI_UINT16_T, mpi_status.MPI_SOURCE, WAIT, 
						dart_sharedmem_comm_list[0]);
			}}
			else{
	...
	if (unitid == PROGRESS_NUM){
		...
		MPI_Send (NULL, 0, MPI_UINT16_T, PROGRESS_UNIT+i, EXIT, dart_sharedmem_comm_list[0]);}
	}
...
#endif
	if (_init_by_dart) {
    DART_LOG_DEBUG("%2d: dart_exit: MPI_Finalize\n", unitid);
		MPI_Finalize();
  }

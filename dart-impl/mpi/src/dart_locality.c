...
#ifdef DART_ENABLE_PAPI
static inline void dart__locality__papi_handle_error(int papi_ret)
{
  /*
  ...
  switch (papi_ret) {
    case PAPI_EINVAL:
         DART_LOG_ERROR("dart_domain_locality: PAPI_EINVAL: "
                        "papi.h is different from the version used to "
                        "compile the PAPI library.");
         break;
    case PAPI_ENOMEM:
         DART_LOG_ERROR("dart_domain_locality: PAPI_ENOMEM: "
    ...
    case PAPI_ESYS:
         DART_LOG_ERROR("dart_domain_locality: PAPI_ESYS: "
                        ...
                        "the errno variable");
         DART_LOG_ERROR("dart_domain_locality: PAPI_ESYS: errno: %d", errno);
         break;
    default:
         DART_LOG_ERROR("dart_domain_locality: PAPI: unknown error: %d",
                        papi_ret);
         break;
  }
}

static inline dart_ret_t dart__locality__papi_init(
  const PAPI_hw_info_t ** hwinfo)
{
  ...
  DART_LOG_DEBUG("dart_domain_locality: PAPI: init");

  papi_ret = PAPI_library_init(PAPI_VER_CURRENT);

  if (papi_ret != PAPI_VER_CURRENT && papi_ret > 0) {
    DART_LOG_ERROR("dart_domain_locality: PAPI: version mismatch");
    return DART_ERR_OTHER;
  } else if (papi_ret < 0) {
    DART_LOG_ERROR("dart_domain_locality: PAPI: init failed, returned %d",
                   papi_ret);
    dart__locality__papi_handle_error(papi_ret);
    return DART_ERR_OTHER;
  } else {
  ...
  if (papi_ret != PAPI_OK) {
    DART_LOG_ERROR("dart_domain_locality: PAPI: PAPI_thread_init failed");
    return DART_ERR_OTHER;
  }

  *hwinfo = PAPI_get_hardware_info();
  if (*hwinfo == NULL) {
    DART_LOG_ERROR("dart_domain_locality: PAPI: get hardware info failed");
    return DART_ERR_OTHER;
  }

  DART_LOG_DEBUG("dart_domain_locality: PAPI: initialized");
  return DART_OK;
}
#endif
...
static inline void dart__locality__domain_locality_init(
  dart_domain_locality_t * loc)
{
  DART_LOG_TRACE("dart__locality__domain_locality_init() loc: %p", loc);
  loc->domain_tag[0]       = '\0';
  ...
  DART_LOG_TRACE("dart__locality__domain_locality_init >");
}
...
{
  /*
   * TODO: Should be cached, domain locality information is invariant.
   */
  ...
  dart__locality__domain_locality_init(loc);
...
#ifdef DART_ENABLE_LIKWID
  DART_LOG_DEBUG("dart_domain_locality: using LIKWID");
  ...
  if (likwid_ret < 0) {
    DART_LOG_ERROR("dart_domain_locality: likwid: topology_init failed, "
                   "returned %d", likwid_ret);
  } else {
...
#ifdef DART_ENABLE_HWLOC
  DART_LOG_DEBUG("dart_domain_locality: using HWLOC");
...
#ifdef DART_ENABLE_PAPI
  DART_LOG_DEBUG("dart_domain_locality: using PAPI");
  ...
  DART_LOG_DEBUG("dart_domain_locality: MIC architecture");
  ...
  if (loc->num_cores < 0) {
    ...
    loc->num_cores = (posix_ret > 0) ? posix_ret : loc->num_cores;
		DART_LOG_DEBUG(
      "dart_domain_locality: POSIX: loc->num_cores = %d", loc->num_cores);
  }
#endif
...
#else
  DART_LOG_ERROR("dart_domain_locality: Linux platform required");
  return DART_ERR_OTHER;
#endif

#ifdef DART_ENABLE_NUMA
  if (loc->num_numa < 0) {
    DART_LOG_DEBUG("dart_domain_locality: using numalib");
...
static inline void dart__locality__unit_locality_init(
  ...
  dart_unit_locality_t ** locality)
{
  ...
  *locality = NULL;
  ...
  dart__locality__unit_locality_init(loc);
  loc->unit_id = unit;

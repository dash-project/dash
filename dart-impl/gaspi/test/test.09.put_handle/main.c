...
int main(int argc, char* argv[])
{
    ...
    *((int *) ptr) = 42;
    ...
    dart_handle_t * handle = (dart_handle_t *) malloc(sizeof(dart_handle_t) * 2);

    if(myid == 1)
    {
        int recv = 1337;
        CHECK(dart_gptr_setunit(&g1, 0));
        CHECK(dart_put_handle(g1, &recv, sizeof(int), &handle[0]));
        gaspi_printf("queue number %d\n", dart_handle_get_queue(handle[0]));
        gaspi_notify(notify_seg, 0, notify_id, notify_value, dart_handle_get_queue(handle[0]), GASPI_BLOCK);

        CHECK(dart_wait_local(handle[0]));
    }
    else if(myid == 0)
    {
    ...
    if(myid == 1)
    {
        ...
        CHECK(dart_put_handle(g2, &recv, sizeof(int), &handle[1]));
        gaspi_printf("queue number %d\n", dart_handle_get_queue(handle[1]));
        gaspi_notify(notify_seg, 0, notify_id, notify_value, dart_handle_get_queue(handle[1]), GASPI_BLOCK);

        CHECK(dart_wait_local(handle[1]));
    }
    else if(myid == 0)
    {
        gaspi_notification_id_t id_received;

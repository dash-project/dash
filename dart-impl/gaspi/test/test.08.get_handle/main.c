...
int main(int argc, char* argv[])
{
    ...
    *((int *) ptr) = 42;
    ...
    if(myid == 0)
    {
        ...
        CHECK(dart_get_handle(&recv, g1, sizeof(int), &handle[0]));
        gaspi_printf("Receive value before wait %d\n",recv);
        CHECK(dart_wait_local(handle[0]));
        gaspi_printf("Receive value after wait %d\n",recv);
    }
    ...
    if(myid == 0)
    {
        ...
        CHECK(dart_get_handle(&recv, g2, sizeof(int), &handle[1]));
        gaspi_printf("Receive value before wait %d\n",recv);

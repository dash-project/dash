...
void dart_buddy_delete(struct dart_buddy * self)
{
    free(self);
}
...
static inline uint64_t _index_offset(int index, int level, int max_level)
{
    return ((index + 1) - (1 << level)) << (max_level - level);
}

static void _mark_parent(struct dart_buddy * self, int index)
{
    for (;;)
    {
...
/**
 * @return memory offset or -1 in error case
 */
uint64_t dart_buddy_alloc(struct dart_buddy * self, size_t s)
{
    int size;
    if (s == 0)
    {
...
static void _combine(struct dart_buddy * self, int index)
{
    for (;;)
    {
        int buddy = index - 1 + (index & 1) * 2;
...
static void _dump(struct dart_buddy * self, int index, int level)
{
    switch (self->tree[index])
    {
        case NODE_UNUSED:

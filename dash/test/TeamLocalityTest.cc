...
TEST_F(TeamLocalityTest, GroupUnits)
{
  ...
  std::vector<dash::local_unit_t> group_1_units;
  std::vector<dash::local_unit_t> group_2_units;
  std::vector<dash::local_unit_t> group_3_units;
  std::vector<std::string> group_1_tags;
  std::vector<std::string> group_2_tags;
  std::vector<std::string> group_3_tags;

  std::vector<dash::local_unit_t> shuffled_unit_ids;
  for (dash::local_unit_t u{0}; u < team.size(); ++u) {
  ...
  for (dash::local_unit_t u : group_1_units) {
    group_1_tags.push_back(tloc.unit_locality(u).domain_tag());
  }
  for (dash::local_unit_t u : group_2_units) {
    group_2_tags.push_back(tloc.unit_locality(u).domain_tag());
  }
  for (dash::local_unit_t u : group_3_units) {
...
TEST_F(TeamLocalityTest, SplitGroups)
{
  ...
  // Put the first 2 units in group 1:
  group_1_units.push_back(dash::local_unit_t{0});
  group_1_units.push_back(dash::local_unit_t{1});
  // Put every second unit in group 2, starting at rank 2:
  for (dash::local_unit_t u{3}; u < team.size(); u += 2) {

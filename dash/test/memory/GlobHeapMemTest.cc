...
TEST_F(GlobHeapMemTest, BalancedAlloc)
{
  ...
  DASH_LOG_TRACE("GlobHeapMemTest.BalancedAlloc", "initial local:",
                 gdmem.local_size());
  DASH_LOG_TRACE("GlobHeapMemTest.BalancedAlloc", "initial global:",
                 gdmem.size());
  ...
  DASH_LOG_TRACE("GlobHeapMemTest.BalancedAlloc", "pre-commit local:",
                 gdmem.local_size());
  DASH_LOG_TRACE("GlobHeapMemTest.BalancedAlloc", "pre-commit global:",
                 gdmem.size());
  ...
  DASH_LOG_TRACE("GlobHeapMemTest.BalancedAlloc", "post-commit local:",
                 gdmem.local_size());
  DASH_LOG_TRACE("GlobHeapMemTest.BalancedAlloc", "post-commit global:",
                 gdmem.size());
...
TEST_F(GlobHeapMemTest, UnbalancedRealloc)
{
  typedef int value_t;
  ...
  if (dash::myid() == 0) {
    ...
    DASH_LOG_TRACE("GlobHeapMemTest.UnbalancedRealloc", "git_first");
    auto git_first  = gdmem.begin();
    DASH_LOG_TRACE("GlobHeapMemTest.UnbalancedRealloc", "git_second");
    auto git_second = git_first + 1;
    DASH_LOG_TRACE("GlobHeapMemTest.UnbalancedRealloc", "git_remote");
    auto git_remote = git_first + gdmem.local_size() + 1;
    DASH_LOG_TRACE("GlobHeapMemTest.UnbalancedRealloc", "git_last");
    auto git_last   = git_first + gdmem.size() - 1;
    DASH_LOG_TRACE("GlobHeapMemTest.UnbalancedRealloc", "git_end");
  ...
  for (dash::team_unit_t unit{0}; unit < dash::Team::All().size();
    ...
    if (dash::myid() != unit) {
      ...
      for(auto it = unit_git_begin; it != unit_git_end; ++it, ++l_idx) {
        ...
        DASH_LOG_TRACE_VAR("GlobHeapMemTest.UnbalancedRealloc", gptr);
...
TEST_F(GlobHeapMemTest, LocalVisibility)
{
  typedef int value_t;
  ...
  dash::barrier();
  DASH_LOG_TRACE("GlobHeapMemTest.LocalVisibility",
  ...
  // for deallocation.
  gdmem.commit();
  DASH_LOG_TRACE("GlobHeapMemTest.LocalVisibility", "commit completed");
  ...
  if (dash::myid() == 0) {
    DASH_LOG_TRACE("GlobHeapMemTest.LocalVisibility", "grow(30)");
    LOG_MESSAGE("grow(30)");
    gdmem.grow(30);
  }
  if (dash::myid() == 1) {
  ...
  if (dash::myid() == 0) {
    ...
    DASH_LOG_TRACE("GlobHeapMemTest.LocalVisibility", "shrink(29)");
    gdmem.shrink(29);
  }
  if (dash::myid() == 1) {
    // marks bucket for detach:
    DASH_LOG_TRACE("GlobHeapMemTest.LocalVisibility", "shrink(30)");
    gdmem.shrink(30);
  }
  DASH_LOG_TRACE("GlobHeapMemTest.LocalVisibility",
...
TEST_F(GlobHeapMemTest, RemoteAccess)
{
  typedef int value_t;
  ...
  // Initialize values in local memory:
  auto lbegin = gdmem.lbegin();
  DASH_LOG_TRACE("GlobHeapMemTest.RemoteAccess",
  ...
  for (dash::team_unit_t u{0}; u < dash::size(); ++u) {
    if (dash::myid() != dash::global_unit_t(u)) {
      ...
      DASH_LOG_DEBUG("GlobHeapMemTest.RemoteAccess",

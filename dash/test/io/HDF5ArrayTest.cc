 ...
 * for example
 */
template <typename T, typename IndexT, typename PatternT>
void fill_array(dash::Array<T, IndexT, PatternT> &array, T secret = 0) {
  std::function<void(const T &, IndexT)> f = [&array, &secret](const T el,
                                                               IndexT i) {
    auto coords = array.pattern().coords(i);
    array[i] = coords[0] + secret;
  };
  dash::for_each_with_index(array.begin(), array.end(), f);
  array.barrier();
}
 ...
 */
template <typename T, typename IndexT, typename PatternT>
void verify_array(dash::Array<T, IndexT, PatternT> &array, T secret = 0) {
  std::function<void(const T &, IndexT)> f = [&array, &secret](T el, IndexT i) {
    auto coords = array.pattern().coords(i);
...
TEST_F(HDF5ArrayTest, OutputStreamOpen) {
  {
    ...
    os << dio::dataset(_dataset) << array_a;
  }
  dash::barrier();
  // Import data
...
TEST_F(HDF5ArrayTest, UnderfilledPattern) {
  ...
  // Verify
  // Check extents
  DASH_ASSERT_EQ(ext_x, array_b.size(),
                 "Array extent does not match input array");
  // Check tilesize
  DASH_ASSERT_EQ(tilesize, array_b.pattern().blocksize(0),
...
TEST_F(HDF5ArrayTest, StreamCreationFlags) {
  ...
  double secret = 10;
  {
    ...
    os << dio::dataset("settwo") << dio::setpattern_key("custom_dash_pattern")
       << dio::store_pattern() << array_a << dio::modify_dataset() << array_a;

    dash::barrier();
  }
  dash::Array<double> array_b;
  InputStream is(_filename);
  is >> dio::dataset("settwo") >> dio::setpattern_key("custom_dash_pattern") >>
      dio::restore_pattern() >> array_b;
...
TEST_F(HDF5ArrayTest, GroupTest) {
  ...
  double secret[] = {10, 11, 12};
  {
    ...
    os << dio::dataset("array_a") << array_a << dio::dataset("g1/array_b")
       << array_b << dio::dataset("g1/g2/array_c") << array_c;
  ...
  dash::Array<double> array_c;
  InputStream is(_filename);
  is >> dio::dataset("array_a") >> array_a >> dio::dataset("g1/array_b") >>
      array_b >> dio::dataset("g1/g2/array_c") >> array_c;
...
TEST_F(HDF5ArrayTest, CustomType) {
...

  {
    ...
    OutputStream os(_filename);
    os << dio::dataset("array_a") << dio::type_converter(converter) << array_a;
...
TEST_F(HDF5ArrayTest, TeamSplit) {
  ...
  LOG_MESSAGE("Splitted team in %d parts, I am %d", num_split,
              static_cast<int>(myteam.position()));
  ...
  if (myteam.position() == 0) {
    {
      ...
      fill_array(array_a, secret);
      myteam.barrier();
      LOG_MESSAGE("Team %d: write array", static_cast<int>(myteam.position()));
      OutputStream os(_filename);
      os << dio::dataset("array_a") << array_a;
      LOG_MESSAGE("Team %d: array written",
                  static_cast<int>(myteam.position()));
      myteam.barrier();
    }
  }
  ...
  if (myteam.position() == 1) {
    ...
    if (array_a.lbegin() != nullptr) {
      LOG_MESSAGE("Team %d: read array", static_cast<int>(myteam.position()));
      InputStream is(_filename);
      is >> dio::dataset("array_a") >> array_a;
      LOG_MESSAGE("Team %d: array read", static_cast<int>(myteam.position()));
      myteam.barrier();

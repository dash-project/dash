...
TEST_F(GraphTest, Construction)
{
  ...
  DASH_LOG_DEBUG("GraphTest.Construction", "construction started");
  ...
  if(dash::myid() == 0) {
    ...
    // all edges from the edge list in the graph
    EXPECT_EQ_U(0, edge_list.size());
    // no edges in the graph that are not in the edge list
    EXPECT_EQ_U(0, unrecognized_edges);
  }
  dash::barrier();
}

...
public:
    ...
    GlobRef<T> get_from_ref(std::array<long long, DIM> coord) const {
        ...
        return m_proxy->m_mat->at(m_proxy->m_mat->m_pattern.index_to_elem(coord));

    }

    Matrix_Ref<T, DIM, 1>()=default;

    GlobRef<T> operator[](size_t n) {
	m_proxy->m_coord[DIM-1]=n;
...
public:
    ...
    {
	Matrix_Ref<ELEMENT_TYPE, DIM, DIM - 1> ref;
        Matrix_RefProxy<ELEMENT_TYPE, DIM> * proxy = new Matrix_RefProxy<ELEMENT_TYPE, DIM>;
	std::fill( proxy->m_coord.begin(), proxy->m_coord.end(),-1);
	ref.m_proxy = proxy;
	ref.m_proxy->m_coord[0] = n;
	ref.m_proxy->m_mat = this;
	...
	return m_pattern.is_local(n, m_myid, dim);
    }

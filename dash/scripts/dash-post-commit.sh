...
mkdir -p $DEBUG_DEPLOY_PATH && (cd $DEBUG_DEPLOY_PATH && \
  ...
  >> build.log 2>&1 && \
  $BASEPATH/dash/scripts/dash-test.sh mpi $DEBUG_DEPLOY_PATH/bin && \
  $BASEPATH/dash/scripts/dash-test.sh shmem $DEBUG_DEPLOY_PATH/bin)
...
mkdir -p $RELEASE_DEPLOY_PATH && (cd $RELEASE_DEPLOY_PATH && \
  ...
  >> build.log 2>&1 && \
  $BASEPATH/dash/scripts/dash-test.sh mpi $RELEASE_DEPLOY_PATH/bin && \
  $BASEPATH/dash/scripts/dash-test.sh shmem $RELEASE_DEPLOY_PATH/bin)

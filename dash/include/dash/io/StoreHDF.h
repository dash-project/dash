...
public:
    ...
    hdf5_file_options foptions = _get_fdefaults())
  {
    ...
    /* local data dims */
    hsize_t data_dimsm[] = {(hsize_t) lfullsize};
    ...
    if (pat_dims == 1) {
      ...
      count[0]      = data_dimsm[0] / tilesize;
      offset[0]      = pattern.local_block(0).offset(0);
      block[0]      = pattern.blocksize(0);
      stride[0]      = pattern.teamspec().extent(0) * block[0];

      //DASH_LOG_DEBUG("UNIT:", dash::myid(), " COUNT:", count[0],
      //							 " OFFSET:", offset[0], " BLOCK:", block[0],
      //							 " STRIDE:", stride[0]);
      ...
      // TODO: Detect if there are any underfilled blocks
      ...
      if (localsize != lfullsize) {
        count[0]    = 1;
        offset[0]   = pattern.local_block(localblocks).offset(0);

...
class HaloMatrixWrapper {
 ...
 public:
  using iterator =
    HaloMatrixIterator<ElementT, PatternT, StencilSpecT, StencilViewScope::ALL>;
  using const_iterator = const iterator;
  using iterator_inner = HaloMatrixIterator<ElementT, PatternT, StencilSpecT,
                                            StencilViewScope::INNER>;
  using const_iterator_inner = const iterator_inner;
  using iterator_bnd = HaloMatrixIterator<ElementT, PatternT, StencilSpecT,
 ...
 public:
  ...
  : _matrix( matrix ), _stencil_spec( stencil_spec ), _cycle_spec( cycle_spec ),
    ...
    _bend( _haloblock, _halomemory, _stencil_spec,
           _haloblock.boundary_size() ) {
    for( const auto& region : _haloblock.halo_regions() ) {
      ...
      if( MemoryArrange == ROW_MAJOR ) {
        if( level == 1 ) {  //|| (level == 2 && region.regionSpec()[0] != 1)) {
          ...
          size_type stride =
            ( num_blocks > 1 )
              ? std::abs( it_dist.lpos().index - it.lpos().index )
              : 1;
        ...
        // TODO more optimizations
          ...
          for( auto& index : halo_data.indexes ) {
            index = static_cast<int>(
              dart_storage<ElementT>( it_tmp.lpos().index - start_index )
                .nelem );

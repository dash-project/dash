...
class Stencil : public Dimensional<int16_t, NumDimensions> {
 ...
 public:
  ...
  : BaseT::Dimensional( value, values... ) {}
...
class RegionCoords : public Dimensional<uint8_t, NumDimensions> {
 ...
 public:
  ...
  template <typename... Values>
  RegionCoords( coord_t value, Values... values )
  : BaseT::Dimensional( value, values... ) {
    _index = index( this->_values );
  }
...
class HaloRegionSpec : public Dimensional<uint8_t, NumDimensions> {
 ...
 public:
  ...
  static constexpr region_index_t MaxIndex = pow(
    RegionCoordsT::REGION_INDEX_BASE, static_cast<udim_t>( NumDimensions ) );
 ...
 public:
  ...
  : _coords( RegionCoordsT( index ) ), _extent( extent ),
    _rel_dim( init_rel_dim() ) {
    init_level();
  }
  ...
  static region_index_t index( const StencilT& stencil ) {
    ...
    for( auto d( 1 ); d < NumDimensions; ++d ) {
      if( stencil[d] < 0 )
        index *= RegionCoordsT::REGION_INDEX_BASE;
      else if( stencil[d] == 0 )
        index = 1 + index * RegionCoordsT::REGION_INDEX_BASE;
      else
        index = 2 + index * RegionCoordsT::REGION_INDEX_BASE;
    }
  ...
  constexpr coord_t operator[]( const region_index_t index ) const {
...
std::ostream& operator<<( std::ostream&                        os,
                          const HaloRegionSpec<NumDimensions>& hrs ) {
...
class HaloSpec {
 ...
 public:
  ...
  HaloSpec( const self& other ) { _specs = other._specs; }
...
class RegionIter
: public std::iterator<std::random_access_iterator_tag, ElementT,
 ...
 public:
  ...
  : _globmem( globmem ), _pattern( pattern ), _region_view( _region_view ),
    _idx( pos ), _max_idx( size - 1 ), _myid( pattern.team().myid() ),
  ...
  std::array<index_type, NumDimensions> gcoords() const {
...
class Region {
 ...
 public:
  Region( const HaloRegionSpecT& region_spec, const ViewSpecT& region,
          GlobMemT& globmem, const PatternT& pattern, const BorderT& border )
...
class HaloBlock {
 private:
  ...
  using HaloExtsMaxT =
    std::array<std::pair<halo_extent_t, halo_extent_t>, NumDimensions>;
  using size_type     = typename PatternT::size_type;
 ...
 public:
  HaloBlock( GlobMemT& globmem, const PatternT& pattern, const ViewSpecT& view,
             const halo_reg_spec_t& halo_reg_spec,
  ...
  : _globmem( globmem ), _pattern( pattern ), _view( view ),
    ...
    for( const auto& spec : _halo_reg_spec.specs() ) {
      ...
      _halo_regions.push_back(
        region_t( spec, ViewSpecT( halo_region_offsets, halo_region_extents ),
                  _globmem, _pattern, border ) );
      _halo_reg_mapping[index] = &_halo_regions.back();
      _boundary_regions.push_back(
        region_t( spec, ViewSpecT( bnd_region_offsets, bnd_region_extents ),
                  _globmem, _pattern, border ) );

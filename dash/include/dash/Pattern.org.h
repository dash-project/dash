...
private:
  ...
  typedef TeamSpec<NumDimensions, Arrangement>   TeamSpec_t;
  typedef AccessBase<NumDimensions, Arrangement> AccessBase_t;
...
private:
  ...
  AccessBase_t       m_accessbase;
  SizeSpec_t         m_sizespec;
...
//  m_sizespec.construct();
    ...
    if (argc_ts == 0)
      m_teamspec = TeamSpec<NumDimensions, Arrangement>(m_team);
  ...
  : m_sizespec(sizespec),
    ...
    m_team(team) {
    m_nunits   = m_team.size();
    m_viewspec = ViewSpec_t(m_sizespec);
    checkValidDistEnum();
    checkTile();
    constructAccessBase();
  }
  ...
  long long atunit_(
    ...
    if (m_teamspec.ndim() == 1) {
      ...
      for (size_t i = 0; i < NumDimensions; i++) {
        index[i] = viewspec.begin(i) + input[i];
        //if (i >= viewspec.view_dim)
        //  index[i] += input[i + NumDimensions - viewspec.view_dim];
  ...
  long long glob_index_to_elem(
    std::array<long long, NumDimensions> input,
    ViewSpec<NumDimensions> & viewspec) const {
    return glob_at_(input, viewspec);
  }
  ...
  long long local_at_(
    std::array<long long, NumDimensions> input,
    ViewSpec<NumDimensions> & local_viewspec) const {
    if (input.size() != NumDimensions) {
      DASH_THROW(
        dash::exception::InvalidArgument,
    ...
    for (size_t i = 0; i < NumDimensions; i++) {
      index[i] = local_viewspec.begin(i) + input[i];
      cyclicfix[i] = 0;
    }
  ...
  TeamSpec<NumDimensions, Arrangement> teamspec() const {
    return m_teamspec;
  }
   ...
   * fuchsto: Rather: Whether the given global index is local to given unit
   *          in given dimension.
   */
  bool is_local(
    size_t idx,
...
private:
  ...
  template<int count>
  void check(const TeamSpec<NumDimensions, Arrangement> & ts) {
    m_teamspec = ts;
    argc_ts++;
  }
  ...
  template<int count>
  void check(const SizeSpec_t & ds) {
    m_sizespec = ds;

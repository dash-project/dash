#ifndef DASH__GRAPH__GLOB_GRAPH_ITER_H__INCLUDED
#define DASH__GRAPH__GLOB_GRAPH_ITER_H__INCLUDED
...
public:
  ...
  : _globmem(nullptr),
    ...
    _idx_bucket_phase(-1)
  {
    DASH_LOG_TRACE_VAR("GlobGraphIter()", _idx);
    DASH_LOG_TRACE_VAR("GlobGraphIter()", _max_idx);
  }
  ...
  : _globmem(gmem),
    ...
    _idx_bucket_phase(0)
  {
    DASH_LOG_TRACE("GlobGraphIter(gmem,idx)", "gidx:", position);
    for (auto unit_bucket_cumul_sizes : *_bucket_cumul_sizes) {
      DASH_LOG_TRACE_VAR("GlobGraphIter(gmem,idx)",
                         unit_bucket_cumul_sizes);
      size_type bucket_cumul_size_prev = 0;
      for (auto bucket_cumul_size : unit_bucket_cumul_sizes) {
        DASH_LOG_TRACE_VAR("GlobGraphIter(gmem,idx)", bucket_cumul_size);
        if (position < bucket_cumul_size) {
          DASH_LOG_TRACE_VAR("GlobGraphIter(gmem,idx)", position);
    ...
    DASH_LOG_TRACE("GlobGraphIter(gmem,idx)",
                   "gidx:",   _idx,
                   "unit:",   _idx_unit_id,
                   "lidx:",   _idx_local_idx,
  ...
  : _globmem(gmem),
    ...
    _idx_bucket_phase(0)
  {
    DASH_LOG_TRACE("GlobGraphIter(gmem,unit,lidx)",
                   "unit:", unit,
                   "lidx:", local_index);
  ...
  GlobGraphIter(
    const GlobGraphIter<E_, G_, P_, R_> & other)
  ...
  {
    DASH_LOG_TRACE_VAR("GlobGraphIter.dart_gptr()", _idx);
    // Create global pointer from unit, bucket and phase:
    dart_gptr_t dart_gptr = _globmem->dart_gptr_at(
                              _idx_unit_id,
                              _idx_bucket_idx,
                              _idx_bucket_phase);
    DASH_LOG_TRACE_VAR("GlobGraphIter.dart_gptr >", dart_gptr);
    return dart_gptr;
  }
  ...
  {
    DASH_LOG_TRACE_VAR("GlobGraphIter.[]()", g_index);
    auto git = *this;
    git  += g_index;
    auto lptr = git.local();
    if (lptr != nullptr) {
      ...
      auto gref = *git;
      DASH_LOG_TRACE_VAR("GlobGraphIter.[] >", gref);
      return gref;
    }
  }
  ...
  template<typename E_, typename G_, typename P_, typename R_>
  inline bool operator<(const GlobGraphIter<E_, G_, P_, R_> & other) const
  {
    return (_idx < other._idx);
  }
...
private:
  ...
  {
    DASH_LOG_TRACE("GlobGraphIter.increment()",
                   "gidx:",   _idx,
                   "unit:",   _idx_unit_id,
                   "lidx:",   _idx_local_idx,
    ...
    if (_idx_local_idx + offset < current_bucket_size) {
      DASH_LOG_TRACE("GlobGraphIter.increment", "position current bucket");
  ...
  {
    DASH_LOG_TRACE("GlobGraphIter.decrement()",
                   "gidx:",   _idx,
                   "unit:",   _idx_unit_id,
                   "lidx:",   _idx_local_idx,
                   "bidx:",   _idx_bucket_idx,
...
#endif // DASH__GRAPH__GLOB_GRAPH_ITER_H__INCLUDED

...
struct InEdgeIteratorWrapper {
  ...
  InEdgeIteratorWrapper(graph_type * graph)
    : _graph(graph),
      _gmem(graph->_glob_mem_in_edge)
  { }
  
  /**
   * Default constructor.
   */
  InEdgeIteratorWrapper() = default;
  ...
  /**
   * Directly gets the attributes of an in-edge identified by a local offset
   */
  ...
  /**
   * Directly sets the attributes of an in-edge identified by a local offset
   */
  ...
  /**
   * Returns the amount of in-edges in the whole graph.
   */
  size_type size() {
    return _gmem->size();
  }
  ...
  /**
   * Returns the amount of in-edges this unit currently holds in local memory
   * space.
   */
  size_type lsize() {
    return _gmem->lsize();
  ...
  /*
   * Returns whether theare are in-edges in global memory space.
   */
  bool empty() {
    return size() == 0;
  }
  ...
  /**
   * Returns the maximum number of in-edges the graph can store.
   */
  size_type max_size() {

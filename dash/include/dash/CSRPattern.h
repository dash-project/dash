...
{
private:
  static const dim_t NumDimensions = 2;

public:
  static constexpr char const * PatternName = "CSRPattern<2>";
...
public:
  ...
  : _arguments(arg, args...),
    ...
    _block_offsets(initialize_block_offsets(
        _local_sizes)),
    _memory_layout(std::array<SizeType, 2> { _size }),
    _blockspec(initialize_blockspec(
        _size,
        _local_sizes)),
    _distspec(_arguments.distspec()),
    ...
    _local_size(
        initialize_local_extent(_team->myid())),
    _local_memory_layout(std::array<SizeType, 2> { _local_size }),
    _local_capacity(initialize_local_capacity())
  {
    DASH_LOG_TRACE("CSRPattern()", "Constructor with argument list");
    DASH_ASSERT_EQ(

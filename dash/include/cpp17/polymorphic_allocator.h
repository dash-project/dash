...
template <>
struct alignas(16) aligned_chunk<32> {
  char x;
};
template <>
struct alignas(16) aligned_chunk<64> {
  char x;
};
...
template <class Tp>
class polymorphic_allocator : public std::scoped_allocator_adaptor<
    ...
    size_t bytes)
{
  ...
  typedef
      typename std::allocator_traits<Allocator>::template rebind_traits<chunk>
                                        chunk_traits;

...
int getdelaylengthfromtime(double delaytime) {
	...
	lapsedtime = Timer::ElapsedSince(starttime) / (double) reps;
	}
	return delaylength;
...
void init(int argc, char **argv)
{
	...
	if (dash::myid() == 0) {
		printf("Running DASH benchmark version 1.0 based on EPCC OpenMP benchmark version 3.1\n"
		   "\t%d thread(s)\n"
		   "\t%d outer repetitions\n"
...
void reference(const char *name, void (*refer)(void)) {
	...
	for (k = 0; k <= outerreps; k++) {
		auto start = Timer::Now();
		refer();
		times[k] = Timer::ElapsedSince(start) / (double) innerreps;
	}
...
void array_delay_dash(int delaylength, dash::Array<double>& a) {
...
void array_delay_dash_local(int delaylength, dash::Array<double>& a) {
...
void array_delay_dash_6500(int delaylength, dash::Array<double>& a) {

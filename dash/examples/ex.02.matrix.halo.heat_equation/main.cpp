...
using matrix_t    = dash::Matrix<double, 2, typename pattern_t::index_type, pattern_t>;
...
int main(int argc, char* argv[]) {
  ...
  pattern_t pattern(size_spec_t(matrix_ext, matrix_ext), dist, tspec,dash::Team::All());
  ...
  dash::HaloSpec<2> halo_s({{ {-1,1}, {-1,1} }});

  dash::HaloMatrix<matrix_t, dash::HaloSpec<2>> halomat(matrix, halo_s);
  dash::HaloMatrix<matrix_t, dash::HaloSpec<2>> halomat2(matrix2, halo_s);

  dash::HaloMatrix<matrix_t, dash::HaloSpec<2>> * current_halo = &halomat;
  dash::HaloMatrix<matrix_t, dash::HaloSpec<2>> * new_halo = &halomat2;
  ...
  for(auto d = 0; d < iterations; ++d)
  {
    ...
    for( auto i = inner_start; i < inner_end; i += offset)
    {
      ...
      for(auto j = 0; j < offset - 2; ++j, ++center, ++center_y_plus, ++center_y_minus)
      {
        auto dtheta = (*(center-1) + *(center+1) - 2 * (*center)) / (dx * dx) +
    ...
    {
      auto core = *it;
      auto dtheta = (it.halo_value(1, -1) + it.halo_value(1, 1) - 2 * core) / (dx * dx) +
    ...
    {
      auto core = *it;
      double dtheta = (it.halo_value(1, -1) + it.halo_value(1, 1) - 2 * core) / (dx * dx) +
                     (it.halo_value(0, -1) + it.halo_value(0, 1) - 2 * core) / (dy * dy);
      *(new_begin + it.lpos()) = core + k * dtheta * dt;
    }
  ...
  for(auto id = 0; id < ranks; ++id)
  {
    ...
    if(id == myid)
    {
      cout << "ID: " << id << " - Total Time: " << diff_total << " - iteration avg: " << diff_total / iterations << endl;
      cout.flush();
    }
  }

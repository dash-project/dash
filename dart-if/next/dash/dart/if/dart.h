 ...
 * \file dart.h
 *
 * DART, The DASH runtime interface, version 4.0.
 *
 ...
 * One can think of a DASH unit like an MPI process or a UPC thread.
 ...
 * that fits (for example, in the context of GPUs and accelerators).
 *
 * Teams are ordered sets of units, identified by an integer ID.  Each
 * unit has a non-negative, zero-based integer ID in a given team,
 * which remains unchanged throughout the lifetime of the team.  In
 ...
 * global address space in a DASH program is a virtual
 * abstraction. Each DASH unit contributes a part of its memory to
 * make up its partition of the global address space. Data items in
 * the global memory are addressed by global pointers provided by the
 * DART runtime.
 *
 * 2) Private and Shared:
 *
 * The adjectives private and shared describe the accessibility of
 * data items in DASH. A shared datum is one that can be accessed by
 * more than one unit (by means of the DART runtime).  A private datum
 * is one that is not shared.
 *
 ...
 * team. The memory allocation is said to be team-aligned (with
 * respect to a specific team), if the same segment-id can be used in
 * a global pointer to refer to any member's portion of the allocated
 * memory.  (See section on global pointers below on segment IDs).
 *
 ...
 * ------------------------
 * All functions in the DART interface shall be implemented in a
 * thread safe way.
 */
...
/*
  --- DART twosided communication ---
*/
#include "dart_twosided.h"

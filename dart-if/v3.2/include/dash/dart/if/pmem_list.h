 ...
 */
#define DART_PMEM_TAILQ_ENTRY(type)\
struct {\
	TOID(type) pe_next;\
	TOID(type) pe_prev;\
}

#define DART_PMEM_TAILQ_HEAD(name, type)\
struct name {\
	TOID(type) pe_first;\
	TOID(type) pe_last;\
}
 ...
 */
#define DART_PMEM_TAILQ_FIRST(head)	((head)->pe_first)
#define DART_PMEM_TAILQ_LAST(head)	((head)->pe_last)

#define DART_PMEM_TAILQ_EMPTY(head)	(TOID_IS_NULL((head)->pe_first))
#define DART_PMEM_TAILQ_NEXT(elm, field)	(D_RO(elm)->field.pe_next)
#define DART_PMEM_TAILQ_PREV(elm, field)	(D_RO(elm)->field.pe_prev)

/*
 * Tail-queue List internal methods.
 */
#define _DART_PMEM_SWAP_PTR_(elm, field) do {\
	TOID_TYPEOF(elm) *elm_ptr = D_RW(elm);\
 ...
 */
#define DART_PMEM_TAILQ_SWAP_HEAD_TAIL(head, field) do {\
...
#define DART_PMEM_TAILQ_FOREACH(var, head, field)\
	for ((var) = DART_PMEM_TAILQ_FIRST(head);\
		!TOID_IS_NULL(var);\
		var = DART_PMEM_TAILQ_NEXT(var, field))

#define DART_PMEM_TAILQ_FOREACH_REVERSE(var, head, field)\
	for ((var) = DART_PMEM_TAILQ_LAST(head);\
		!TOID_IS_NULL(var);\
		var = DART_PMEM_TAILQ_PREV(var, field))

#define DART_PMEM_TAILQ_INIT(head) do {\
...
#define DART_PMEM_TAILQ_INSERT_HEAD(head, elm, field) do {\
	TOID_TYPEOF(elm) *elm_ptr = D_RW(elm);\
...
#define DART_PMEM_TAILQ_INSERT_TAIL(head, elm, field) do {\
	TOID_TYPEOF(elm) *elm_ptr = D_RW(elm);\
...
#define DART_PMEM_TAILQ_INSERT_AFTER(listelm, elm, field) do {\
	TOID_TYPEOF(elm) *elm_ptr = D_RW(elm);\
...
#define DART_PMEM_TAILQ_INSERT_BEFORE(listelm, elm, field) do {\
	TOID_TYPEOF(elm) *elm_ptr = D_RW(elm);\
	TOID_TYPEOF(listelm) *listelm_ptr = D_RW(listelm);\
	TX_ADD_DIRECT(&elm_ptr->field);\
...
#define DART_PMEM_TAILQ_REMOVE(head, elm, field) do {\
	TOID_TYPEOF(elm) *elm_ptr = D_RW(elm);\
...
#define DART_PMEM_TAILQ_REMOVE_FREE(head, elm, field) do {\
	DART_PMEM_TAILQ_REMOVE(head, elm, field);\
	TX_FREE(elm);\
} while (0)
 ...
 */
#define DART_PMEM_TAILQ_MOVE_ELEMENT_HEAD(head, elm, field) do {\
	TOID_TYPEOF(elm) *elm_ptr = D_RW(elm);\
	if (TOID_EQUALS((head)->pe_last, elm) &&\
		TOID_EQUALS(D_RO((head)->pe_first)->field.pe_next, elm)) {\
		_DART_PMEM_SWAP_PTR_(elm, field);\
		_DART_PMEM_SWAP_PTR_((head)->pe_first, field);\
		DART_PMEM_TAILQ_SWAP_HEAD_TAIL(head, field);\
	} else {\
		TOID_TYPEOF(elm) *prev = D_RW(elm_ptr->field.pe_prev);\
		TX_ADD_DIRECT(&prev->field.pe_next);\
		prev->field.pe_next = elm_ptr->field.pe_next;\
...
#define DART_PMEM_TAILQ_MOVE_ELEMENT_TAIL(head, elm, field) do {\
	TOID_TYPEOF(elm) *elm_ptr = D_RW(elm);\
	if (TOID_EQUALS((head)->pe_first, elm) &&\
		TOID_EQUALS(D_RO((head)->pe_last)->field.pe_prev, elm)) {\
		_DART_PMEM_SWAP_PTR_(elm, field);\
		_DART_PMEM_SWAP_PTR_((head)->pe_last, field);\
		DART_PMEM_TAILQ_SWAP_HEAD_TAIL(head, field);\
	} else {\
		TOID_TYPEOF(elm) *next = D_RW(elm_ptr->field.pe_next);\
		TX_ADD_DIRECT(&next->field.pe_prev);\
